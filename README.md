...
xhs login --qrcode                    # Blocking headless QR login for streaming terminals
xhs login --qrcode --async            # Agent-safe: print QR, then continue in a detached worker
xhs login --qrcode-status             # Check the detached QR worker
...
1. **Saved cookies** — loads from `~/.xiaohongshu-cli/cookies.json`; set `XHS_CONFIG_DIR` to relocate all persistent CLI state, or use the global `--cookie-file PATH` option or `XHS_COOKIE_FILE` for cookies only
...
In a sandbox without a GUI, run `xhs auth` for the manual external-browser flow. The CLI prints the XHS login URL, waits for a hidden prompt, and saves the pasted browser `Cookie` request header. In browser developer tools, copy the `Cookie` header from an authenticated request to `xiaohongshu.com`; do not paste a `Set-Cookie` response header.

For a non-streaming agent tool call, use `xhs login --qrcode --async`. It starts a detached Camoufox worker, waits only until the QR is ready, prints both dark- and light-background variants, and exits so the tool result can be shown. After scanning and confirming in the Xiaohongshu app, run `xhs login --qrcode-status`; once it reports `succeeded`, run `xhs status`. The sandbox must remain alive and preserve detached processes between commands. Use blocking `xhs login --qrcode` in one-shot containers or runners that kill the complete process tree when the command exits.
...
# After confirmation, check the worker, then verify the persisted session:
...
For automation, `XHS_CONFIG_DIR=/data/xhs` keeps cookies, QR-login state, and token, index, and search-session caches in one mounted directory. Use `XHS_COOKIE_FILE=/data/xhs/cookies.json` when only the cookie file should move; it takes precedence over `XHS_CONFIG_DIR` for cookies. `XHS_COOKIES` can supply the import value without putting it directly in the command arguments. Treat cookie values and the saved JSON file as secrets.
...
| `XHS_CONFIG_DIR` | `~/.xiaohongshu-cli` | Directory for cookies, `qr_login.json`, and token, index, and search-session caches |
| `XHS_COOKIE_FILE` | `$XHS_CONFIG_DIR/cookies.json` | Cookie-only path override used by load, save, and logout; takes precedence over `XHS_CONFIG_DIR` |
...
xhs login --qrcode                    # 阻塞式无头二维码登录（适合流式终端）
xhs login --qrcode --async            # Agent 模式：输出二维码后由后台进程继续登录
xhs login --qrcode-status             # 检查后台二维码登录状态
...
1. **已保存 Cookie** — 默认从 `~/.xiaohongshu-cli/cookies.json` 加载；可用 `XHS_CONFIG_DIR` 移动全部持久化状态，或通过全局 `--cookie-file PATH` / `XHS_COOKIE_FILE` 仅指定 Cookie 路径
...
无 GUI 的沙箱可以直接执行 `xhs auth`：CLI 会输出小红书登录页地址并以隐藏输入方式等待 Cookie。请在外部浏览器登录后，从开发者工具中复制发往 `xiaohongshu.com` 的已认证请求的 `Cookie` 请求头；不要复制响应中的 `Set-Cookie`。

非流式 Agent 工具调用应使用 `xhs login --qrcode --async`。命令会启动独立的 Camoufox 后台进程，只等待二维码生成，然后输出适用于深色和浅色背景的两个版本并退出，使 Agent 能先把工具结果返回给用户。扫码并在小红书 App 中确认后，执行 `xhs login --qrcode-status`；状态为 `succeeded` 后再执行 `xhs status`。沙箱必须在多次命令之间保持运行，并且不能在首个命令退出时杀死整个进程树；一次性容器应继续使用阻塞式 `xhs login --qrcode`。
...
# 确认登录后检查后台任务，再验证已持久化的会话：
XHS_CONFIG_DIR=/data/xhs xhs login --qrcode-status
XHS_CONFIG_DIR=/data/xhs xhs status
```

Cookie 保存后有效期 **7 天**，超时后自动尝试从浏览器刷新。挂载整个配置目录时可设置 `XHS_CONFIG_DIR=/data/xhs`，Cookie、二维码登录状态 `qr_login.json`、token、短索引和搜索会话缓存都会写入该目录。如只需移动 Cookie，可设置 `XHS_COOKIE_FILE` 或把全局参数放在子命令前，例如 `xhs --cookie-file /data/xhs/cookies.json status`；Cookie 专用路径优先于 `XHS_CONFIG_DIR`。也可通过 `XHS_COOKIES` 传入待导入的 Cookie，避免把密钥直接写入命令参数。Cookie 和保存的 JSON 文件都应按密钥保护。
